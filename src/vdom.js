...
class VNode {
	...
	constructor({ node, parentVNode, vApp, bindings }) {
		...
		if (vNode.$node.nodeType == Node.ELEMENT_NODE) {
			...
			if (!isTemplate) {
				for (const childNode of Array.from(vNode.$node.childNodes)) {
	...
	render(vRenderContext) {
		...
		if (vNode.isComponent) {
			...
			for (const childNode of Array.from(fragmentNode.childNodes)) {
		...
		if (vFor != undefined) {
			...
			vForExp.execute(function(bindings) {
				...
				if (newNodes instanceof DocumentFragment) {
					let l = [];
					for (const node of Array.from(newNodes.childNodes)) {
						l.push(node);
					}
					newNodes = l;
				}
